...
from scrapy import signals, Spider  # type: ignore
from scrapy.crawler import CrawlerProcess

from items.custom_item import CustomItem
...
class Signals:
    ...
    def on_item_scraped(self, item: CustomItem) -> None:
        if not self.queue.is_closed:
            self.queue.put(item.toJson())
        else:
            logger.info(
...
class PipelineRunner(Runnable):
    ...
    def run(
        ...
        **kwargs: P.kwargs,
        ...
        def run() -> None:
            try:
                self.pipeline.run(qdrant_adapter(self.scraping_resource, embed=["title", "question"]), **kwargs)  # type: ignore[arg-type]
            except Exception:
